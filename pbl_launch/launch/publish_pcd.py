...
def load_pcd(path: str) -> PointCloud2:
    """Load an ASCII or binary PCD file and return a PointCloud2 message."""
    ...
    if not counts_str and fields_str:
        counts_str = ["1"] * len(fields_str)
    ...
    if data_format == "ascii":
        data_text = raw[header_end:].decode("utf-8")
        data_lines = data_text.splitlines()
        expected_points = width * height if width and height else sum(1 for line in data_lines if line.strip())
        ...
        actual_points = min(expected_points, len(raw[header_end:]) // point_step)
        data = bytearray(raw[header_end : header_end + actual_points * point_step])
    else:
        raise ValueError(f"Only ASCII and binary PCD are supported, got: {data_format}")

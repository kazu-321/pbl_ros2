...
void pbl_wheel_odometry::joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  ...
  if (right_wheel_index == msg->name.size() || left_wheel_index == msg->name.size()) {
    RCLCPP_WARN_THROTTLE(
      ...
      "Could not find joint0/joint1 in /joint_states.");
    return;
  }

  const bool has_positions =
    right_wheel_index < msg->position.size() && left_wheel_index < msg->position.size();
  const bool has_velocities =
    right_wheel_index < msg->velocity.size() && left_wheel_index < msg->velocity.size();

...
pbl_occupancy_grid_publisher::pbl_occupancy_grid_publisher (const rclcpp::NodeOptions &options)
    : Node ("pbl_occupancy_grid_publisher", options),
      ...
      occupied_count_full_score_ (static_cast<int> (this->declare_parameter<int32_t> ("occupied_count_full_score", kDefaultOccupiedCountFullScore))),
      transform_timeout_sec_ (this->declare_parameter<double> ("transform_timeout_sec", 0.1)) {
    tf_buffer_.setUsingDedicatedThread (true);

    occupancy_pub_      = this->create_publisher<nav_msgs::msg::OccupancyGrid> ("occupancy_grid", rclcpp::QoS (1).reliable ().transient_local ());
    used_map_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2> ("used_map_cloud", rclcpp::QoS (1).reliable ().transient_local ());

    const auto map_qos = rclcpp::QoS (1).reliable ().transient_local ();
    // PointCloud2 sources such as LiDAR drivers normally use SensorDataQoS
    // (best effort). Match that QoS so live clouds can update the occupancy grid.
    const auto cloud_qos = rclcpp::SensorDataQoS ();
    map_sub_             = this->create_subscription<sensor_msgs::msg::PointCloud2> (map_topic_, map_qos, std::bind (&pbl_occupancy_grid_publisher::map_callback, this, std::placeholders::_1));
    cloud_sub_           = this->create_subscription<sensor_msgs::msg::PointCloud2> (cloud_topic_, cloud_qos, std::bind (&pbl_occupancy_grid_publisher::cloud_callback, this, std::placeholders::_1));

    timer_ = this->create_wall_timer (std::chrono::seconds (1), std::bind (&pbl_occupancy_grid_publisher::timer_callback, this));
...
void pbl_occupancy_grid_publisher::map_callback (const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    RCLCPP_INFO (this->get_logger (), "Initialized occupancy grid from %u x %u cells", occupancy_grid_.info.width, occupancy_grid_.info.height);
}

void pbl_occupancy_grid_publisher::cloud_callback (const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    std::lock_guard<std::mutex> lock (mutex_);
    if (!map_initialized_) {
        return;
    }
    ...
    if (!has_required_fields (*msg)) {
        RCLCPP_WARN_THROTTLE (this->get_logger (), *this->get_clock (), 3000, "Filtered cloud does not have x/y/z fields.");
        return;
    }
...
bool pbl_occupancy_grid_publisher::has_required_fields (const sensor_msgs::msg::PointCloud2 &msg) const {
    return has_field (msg, "x") && has_field (msg, "y") && has_field (msg, "z");
}

bool pbl_occupancy_grid_publisher::transform_pointcloud (const sensor_msgs::msg::PointCloud2 &in, sensor_msgs::msg::PointCloud2 &out, const std::string &target_frame, const rclcpp::Time &stamp) const {
    try {
        ...
        const auto transform = tf_buffer_.lookupTransform (target_frame, in.header.frame_id, stamp, tf2::durationFromSec (transform_timeout_sec_));
        tf2::doTransform (in, out, transform);
        out.header.stamp    = in.header.stamp;
        out.header.frame_id = target_frame;
        return true;
    } catch (const tf2::TransformException &ex) {
        RCLCPP_WARN_THROTTLE (this->get_logger (), *this->get_clock (), 3000, "Failed to transform PointCloud2 from '%s' to '%s': %s", in.header.frame_id.c_str (), target_frame.c_str (), ex.what ());
        return false;
    }
}

void pbl_occupancy_grid_publisher::initialize_grid_from_map (const sensor_msgs::msg::PointCloud2 &map_cloud) {
    ...
    const std::size_t width    = static_cast<std::size_t> (std::ceil ((max_x - origin_x) / grid_resolution_)) + 1;
    const std::size_t height   = static_cast<std::size_t> (std::ceil ((max_y - origin_y) / grid_resolution_)) + 1;
...
void pbl_occupancy_grid_publisher::build_static_occupancy_from_map (const sensor_msgs::msg::PointCloud2 &map_cloud) {
    ...
    std::vector<int>                             cell_counts (static_cast<std::size_t> (occupancy_grid_.info.width) * static_cast<std::size_t> (occupancy_grid_.info.height), 0);
    sensor_msgs::PointCloud2ConstIterator<float> x_it (map_cloud, "x");
    sensor_msgs::PointCloud2ConstIterator<float> y_it (map_cloud, "y");
    sensor_msgs::PointCloud2ConstIterator<float> z_it (map_cloud, "z");
    const std::size_t                            point_count = static_cast<std::size_t> (map_cloud.width) * map_cloud.height;
    ...
    for (std::size_t idx = 0; idx < cell_counts.size (); ++idx) {
        ...
        static_occupancy_[idx] = to_grid_value (count_to_occupancy (cell_counts[idx], occupied_count_full_score_));
    }
}

sensor_msgs::msg::PointCloud2 pbl_occupancy_grid_publisher::build_used_map_cloud (const sensor_msgs::msg::PointCloud2 &map_cloud) const {
    ...
    for (std::size_t i = 0; i < point_count; ++i, ++x_it, ++y_it, ++z_it) {
        ...
        used.data.insert (used.data.end (), map_cloud.data.begin () + static_cast<std::ptrdiff_t> (offset), map_cloud.data.begin () + static_cast<std::ptrdiff_t> (offset + map_cloud.point_step));
    }
...
void pbl_occupancy_grid_publisher::append_cloud_buffer (const sensor_msgs::msg::PointCloud2 &cloud_in_map) {
    ...
    cloud_buffer_.data.insert (cloud_buffer_.data.end (), cloud_in_map.data.begin (), cloud_in_map.data.end ());
    cloud_buffer_.width += cloud_in_map.width;
...
bool pbl_occupancy_grid_publisher::world_to_cell (double x, double y, std::size_t &x_idx, std::size_t &y_idx) const {
    if (occupancy_grid_.info.width == 0 || occupancy_grid_.info.height == 0) {
        return false;
    }
...
void pbl_occupancy_grid_publisher::rebuild_occupancy_grid (const sensor_msgs::msg::PointCloud2 &cloud_in_map) {
    ...
    for (std::size_t idx = 0; idx < total_cells; ++idx) {
        ...
        occupancy_grid_.data[idx] = to_grid_value (std::max<int> (static_cast<int> (occupancy_grid_.data[idx]), dynamic_value));
    }
}

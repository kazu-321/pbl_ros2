...
bool has_field (const sensor_msgs::msg::PointCloud2 &msg, const std::string &field_name) {
    return std::any_of (msg.fields.begin (), msg.fields.end (), [&field_name] (const sensor_msgs::msg::PointField &field) { return field.name == field_name; });
}
}  // namespace
...
pbl_pointcloud2_filter::pbl_pointcloud2_filter (const rclcpp::NodeOptions &options)
    : Node ("pbl_pointcloud2_filter", options),
      ...
      output_topic_transformed_ (this->declare_parameter<std::string> ("output_topic_transformed", "/unilidar/cloud_filtered_transformed")),
      navigation_output_topic_ (this->declare_parameter<std::string> ("navigation_output_topic", "/unilidar/cloud_navigation")),
      navigation_lidar_frame_id_ (this->declare_parameter<std::string> ("navigation_lidar_frame_id", "unilidar_lidar")),
      ...
      navigation_min_height_ (this->declare_parameter<double> ("navigation_min_height", 0.0)),
      navigation_max_height_ (this->declare_parameter<double> ("navigation_max_height", 0.6)) {
    ...
    collision_boxes_.push_back (CollisionBox{"body", body_center_x, body_center_y, body_center_z, body_size_x, body_size_y, body_size_z});
    collision_boxes_.push_back (CollisionBox{"pc", pc_center_x, pc_center_y, pc_center_z, pc_size_x, pc_size_y, pc_size_z});

    const auto cloud_qos            = rclcpp::QoS (10);
    filtered_cloud_pub_             = this->create_publisher<sensor_msgs::msg::PointCloud2> (output_topic_, cloud_qos);
    filtered_cloud_transformed_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2> (output_topic_transformed_, cloud_qos);
    navigation_cloud_pub_           = this->create_publisher<sensor_msgs::msg::PointCloud2> (navigation_output_topic_, cloud_qos);
    collision_marker_pub_           = this->create_publisher<visualization_msgs::msg::MarkerArray> ("collision_boxes", rclcpp::QoS (1).reliable ());
    cloud_sub_                      = this->create_subscription<sensor_msgs::msg::PointCloud2> (input_topic_, cloud_qos, std::bind (&pbl_pointcloud2_filter::pointcloud_callback, this, std::placeholders::_1));
    collision_marker_timer_         = this->create_wall_timer (std::chrono::milliseconds (10), std::bind (&pbl_pointcloud2_filter::publish_collision_markers, this));
    ...
    RCLCPP_INFO (this->get_logger (), "output_topic_transformed: %s", output_topic_transformed_.c_str ());
    RCLCPP_INFO (this->get_logger (), "navigation_output_topic: %s", navigation_output_topic_.c_str ());
    RCLCPP_INFO (this->get_logger (), "navigation_lidar_frame_id: %s", navigation_lidar_frame_id_.c_str ());
    RCLCPP_INFO (this->get_logger (), "navigation_height: %.3f to %.3f", navigation_min_height_, navigation_max_height_);
    ...
    for (const auto &box : collision_boxes_) {
        RCLCPP_INFO (this->get_logger (), "collision_box[%s]: center=(%.3f, %.3f, %.3f) size=(%.3f, %.3f, %.3f)", box.name.c_str (), box.center_x, box.center_y, box.center_z, box.size_x, box.size_y, box.size_z);
    }
...
void pbl_pointcloud2_filter::pointcloud_callback (const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    if (!has_required_fields (*msg)) {
        RCLCPP_WARN_THROTTLE (this->get_logger (), *this->get_clock (), 3000, "PointCloud2 does not have x/y/z fields. Passing the cloud through unchanged.");
    ...
    try {
        if (original_frame_id == target_frame_id_) {
            ...
            const auto transform = tf_buffer_.lookupTransform (target_frame_id_, original_frame_id, tf2::TimePointZero, tf2::durationFromSec (transform_timeout_sec_));
            tf2::doTransform (*msg, cloud_in_target, transform);
        }
        cloud_in_target.header.stamp = msg->header.stamp;
    } catch (const tf2::TransformException &ex) {
        RCLCPP_WARN_THROTTLE (this->get_logger (), *this->get_clock (), 3000, "Failed to transform PointCloud2 from '%s' to '%s': %s", msg->header.frame_id.c_str (), target_frame_id_.c_str (), ex.what ());
        return;
    }
    ...
    double lidar_x                  = 0.0;
    bool   lidar_position_available = true;
    try {
        if (navigation_lidar_frame_id_ != target_frame_id_) {
            const auto lidar_transform = tf_buffer_.lookupTransform (target_frame_id_, navigation_lidar_frame_id_, tf2::TimePointZero, tf2::durationFromSec (transform_timeout_sec_));
            lidar_x                    = lidar_transform.transform.translation.x;
        }
    } catch (const tf2::TransformException &ex) {
        RCLCPP_WARN_THROTTLE (this->get_logger (), *this->get_clock (), 3000, "Failed to get LiDAR position from '%s' to '%s': %s", navigation_lidar_frame_id_.c_str (), target_frame_id_.c_str (), ex.what ());
        lidar_position_available = false;
    }

    if (lidar_position_available) {
        auto navigation_cloud         = filter_navigation_points (cloud_in_target, lidar_x);
        navigation_cloud.header.stamp = msg->header.stamp;
        navigation_cloud_pub_->publish (navigation_cloud);
    }

    if (original_frame_id != target_frame_id_) {
        try {
            const auto                    transform_back = tf_buffer_.lookupTransform (original_frame_id, target_frame_id_, tf2::TimePointZero, tf2::durationFromSec (transform_timeout_sec_));
            ...
            RCLCPP_WARN_THROTTLE (this->get_logger (), *this->get_clock (), 3000, "Failed to transform filtered PointCloud2 back from '%s' to '%s': %s", target_frame_id_.c_str (), original_frame_id.c_str (), ex.what ());
            return;
        }
    } else {
...
void pbl_pointcloud2_filter::publish_collision_markers () {
    ...
    for (std::size_t i = 0; i < collision_boxes_.size (); ++i) {
        ...
        const std::array<geometry_msgs::msg::Point, 8> corners = {
            make_corner (box.center_x - hx, box.center_y - hy, box.center_z - hz), make_corner (box.center_x + hx, box.center_y - hy, box.center_z - hz), make_corner (box.center_x + hx, box.center_y + hy, box.center_z - hz),
            make_corner (box.center_x - hx, box.center_y + hy, box.center_z - hz), make_corner (box.center_x - hx, box.center_y - hy, box.center_z + hz), make_corner (box.center_x + hx, box.center_y - hy, box.center_z + hz),
            make_corner (box.center_x + hx, box.center_y + hy, box.center_z + hz), make_corner (box.center_x - hx, box.center_y + hy, box.center_z + hz),
        };
...
bool pbl_pointcloud2_filter::is_inside_box (double x, double y, double z, const CollisionBox &box) const {
    ...
    return std::abs (x - box.center_x) <= half_x && std::abs (y - box.center_y) <= half_y && std::abs (z - box.center_z) <= half_z;
}

sensor_msgs::msg::PointCloud2 pbl_pointcloud2_filter::filter_self_points (const sensor_msgs::msg::PointCloud2 &cloud) const {
    ...
    for (std::size_t i = 0; i < point_count; ++i, ++x_it, ++y_it, ++z_it) {
        ...
        filtered.data.insert (filtered.data.end (), cloud.data.begin () + static_cast<std::ptrdiff_t> (offset), cloud.data.begin () + static_cast<std::ptrdiff_t> (offset + cloud.point_step));
    }
...
sensor_msgs::msg::PointCloud2 pbl_pointcloud2_filter::filter_navigation_points (const sensor_msgs::msg::PointCloud2 &cloud, double lidar_x) const {
    if (cloud.point_step == 0 || cloud.width == 0 || cloud.height == 0) {
        return cloud;
    }
    ...
    for (std::size_t i = 0; i < point_count; ++i, ++x_it, ++y_it, ++z_it) {
        ...
        if (x < lidar_x || z < navigation_min_height_ || z > navigation_max_height_) {
            continue;
        }
